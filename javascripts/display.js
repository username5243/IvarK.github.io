...
function infinityUpgradesDisplay(){
	for (let x = 1; x <= 4; x++) {
		for (let y = 1; y <= 4; y++) {
			...
			document.getElementById("infi" + id).className = "infinistorebtn" + (player.infinityUpgrades.includes(upgId) ? "bought" : INF_UPGS.normal.can(id) ? y : "locked")
		}
	}
...
function breakInfinityUpgradeDisplay(){
	...
	if (player.dimensionMultDecrease > 3) document.getElementById("postinfi42").innerHTML = "Decrease the Dimension cost multiplier increase post-e308<br>" + player.dimensionMultDecrease + "x -> " + (player.dimensionMultDecrease - 1) + "x<br>Cost: " + shorten(player.dimensionMultDecreaseCost) +" IP"
...
function breakNGm2UpgradeRow6Display(){
	...
	document.getElementById("postinfi60").innerHTML = "You gain more " + (player.tickspeedBoosts ? "Galaxy Points" : "antimatter") + " based on your galaxies." + (player.tickspeedBoosts ? "" : "<br>Currently: " + shorten(getNewB60Mult()) + "x") + "<br>Cost: " + shortenCosts(1e50) + " IP"
...
function breakEternityDisplay(){
	...
	for (var u = 1; u <= (player.achievements.includes("ng3p101") ? 13 : player.ghostify.ghostlyPhotons.unl ? 10 : 7); u++) {
		...
		if (u == 8 || u == 13) document.getElementById("breakUpg" + u + "Mult").textContent = (getBreakUpgMult(u) * 100 - 100).toFixed(1)
		else if (u != 7 && u <= 10) document.getElementById("breakUpg" + u + "Mult").textContent = shortenMoney(getBreakUpgMult(u))
		else if (u == 12) document.getElementById("breakUpg" + u + "Mult").textContent = shorten(getBreakUpgMult(u))
	}
...
function updateDimensionsDisplay() {
	if (document.getElementById("dimensions").style.display == "block") {
		...
		if (document.getElementById("gdims").style.display == "block") GDs.updateDisplayOnTick()
	}
	tickspeedDisplay()
...
function replicantiDisplay() {
	if (player.replicanti.unl) {
		...
		document.getElementById("replicantiinterval").innerHTML = "Interval: "+timeDisplayShort(Decimal.div(tmp.rep.interval, 100), true, 3) + (isIntervalAffordable() ? "<br>-> "+timeDisplayShort(Decimal.div(tmp.rep.interval, 100 / 0.9), true, 3)+" Cost: "+shortenCosts(player.replicanti.intervalCost)+" IP" : "")
